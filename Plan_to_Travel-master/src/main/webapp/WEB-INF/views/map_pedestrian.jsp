...
<body>
    <div id="map_div_ped">
        <input type="hidden" id="searchAddress_ped" /> <input type="hidden" id="startx_ped" /> <input type="hidden"
            id="starty_ped" /> <input type="hidden" id="endx_ped" /> <input type="hidden" id="endy_ped" />
        <div class="_map_layer_overlay">
            <div class="__space_15_h"></div>
            <div class="_map_overlay_row">
                <input type="text" id="searchStartAddress_ped" class="_search_entry _search_entry_short"
                    placeholder="출발지를 입력하세요" onkeyup="onKeyupSearchPoi_ped(this);">
                <button onclick="clickSearchPois_ped(this);" class="_search_address_btn_ped btn btn-primary btn-sm"
                    style="margin-bottom: 14px; pointer-events: all; cursor: pointer;">
                    출발</button>
                <div class="__space_13_w"></div>
                <input type="text" id="searchEndAddress_ped" class="_search_entry _search_entry_short"
                    placeholder="목적지를 입력하세요" onkeyup="onKeyupSearchPoi_ped(this);">
                <button onclick="clickSearchPois_ped(this);" class="_search_address_btn_ped btn btn-primary btn-sm"
                    style="margin-top: 53px; margin-bottom: 14px; pointer-events: all; cursor: pointer;">
                    도착</button>
                <div class="__space_10_w"></div>
                <button class="_btn_action_ped _btn_action-search __color_grey btn btn-primary btn-sm"
            ...
            <div id="wpList">
                ...
                <div class="waypoint_input _map_overlay_row" data-idx_ped="0">
                    <input type="hidden" name="multipos" /> <input type="text"
                        class="_search_entry_text _search_entry_short" style="margin-top: 10px;"
                        onkeyup="onKeyupSearchPoi_ped(this);" placeholder="경유지를 입력하세요.">
                    <button onclick="clickSearchPois_ped(this);" class="_search_address_btn_ped btn btn-primary btn-sm"
                ...
                success: function (response) {
                    ...
                    if (response.resultData.header) {
                        ...
                        for (var i in resultlinkPoints_ped) {
                            var lineLatLng_ped = new Tmapv2.LatLng(resultlinkPoints_ped[i].location
                                .latitude, resultlinkPoints_ped[i].location.longitude);
            ...
            var params_ped = {
                ...
                `;
                    ...
                    for (var k in resultpoisData_ped) {
                        ...
                        var fullAddressRoad = resultpoisData_ped[k].newAddressList.newAddress[0]
                        .fullAddressRoad;
                        ...
                        \${(resultpoisData_ped.length-1) == Number(k) ? "": ` < div class = "_search_item_split" > <
                            /div>`}
                        `;
 ...
 */                            
                if(tel_ped !== "") {
                	content_ped += ` < div class = "_tmap_preview_popup_address" > \$ {
                            tel_ped
                        } < /div>`;
                    }
        ...
        function routesPedestrian_ped() {
            return new Promise(function (resolve_ped, reject_ped) {
                ...
                tData_ped.getRoutePlanForPeopleJson(startLatLng_ped, endLatLng_ped, "출발지", "도착지", optionObj_ped,
                    params_ped);
            });
        }
        ...
        async function routesCarInit_ped() {
            ...
            for (var mode_ped of modes_ped) {
                await routesCar_ped(mode_ped)
                    .then(v => {
                        console.log("complete routePlan: mode_ped: ", v);
                    });
            ...
            return new Promise(function (resolve_ped, reject_ped) {
                ...
                var params_ped = {
                    ...
                    `;
                        ...
                        if (drawMode_ped == "apiRoutesCar_" + mode_ped || drawMode_ped ==
                            "apiRoutesMulti_" + mode_ped) {
                            reset_ped();
                            var positionBounds_ped = new Tmapv2
                        .LatLngBounds(); //맵에 결과물 확인 하기 위한 LatLngBounds객체 생성
                            for (var i in resultData_ped) { //for문 [S]
                                ...
                                if (geometry_ped.type == "LineString") {
                                    ...
                                    for (var j in geometry_ped.coordinates) {
                                        var latlng = new Tmapv2.LatLng(geometry_ped.coordinates[j][1],
                                    ...
                                    var markerPosition_ped = new Tmapv2.LatLng(geometry_ped.coordinates[
                                        1], geometry_ped.coordinates[0]);
            ...
            if ($(btn).hasClass('wp_clear')) {
                ...
                $(".waypoint_input").each(function (idx_ped) {
                    ...
                    markerWp_ped[idx_ped] = new Tmapv2.Marker({
                        ...
                        icon: "http://tmapapi.sktelecom.com/upload/tmap/marker/pin_b_m_" + idx_ped +
                            ".png",
                        iconSize: new Tmapv2.Size(24, 38),
                        map: map_ped
                    });
                            ...
                            for (var i in resultData) { //for문 [S]
                                ...
                                if (geometry.type == "LineString") {
                                    for (var j in geometry.coordinates) {
                                        ...
                                        // 포인트 객체를 받아 좌표값으로 변환
                                        var convertPoint = new Tmapv2.Projection
